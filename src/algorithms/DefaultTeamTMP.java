...
// ... (votre code existant)
  ...
  */
  ...
  public Circle welzl(ArrayList<Point> points) {
    ...
    ArrayList<Point> support = new ArrayList<>();
    return welzlRecursive(shuffledPoints, support);
  }

  private Circle welzlRecursive(ArrayList<Point> points, ArrayList<Point> support) {
    if (points.isEmpty() || support.size() == 3) {
      return minidiskWithTwoOrThreePoints(support);
    }

    ArrayList<Point> remaining = new ArrayList<>(points);
    Point p = remaining.remove(remaining.size() - 1);

    Circle d = welzlRecursive(remaining, support);
    ...
    support.add(p);
    return welzlRecursive(remaining, support);
  }

  private Circle minidiskWithTwoOrThreePoints(ArrayList<Point> support) {
    if (support.isEmpty()) {
      return null;
    } else if (support.size() == 1) {
      return new Circle(support.get(0), 0);
    } else if (support.size() == 2) {
      Point p1 = support.get(0);
      Point p2 = support.get(1);
      return new Circle(new Point((p1.x + p2.x) / 2, (p1.y + p2.y) / 2), p1.distance(p2) / 2);
    } else {
      // Trois points
      Point p1 = support.get(0);
      Point p2 = support.get(1);
      Point p3 = support.get(2);
